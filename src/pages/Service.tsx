...
export default function main(){
    ...
    <div className='text-text-secondary flex flex-row p-10 font-main'>
        ...
        <div className='flex-5'>
            ...
            <div>
                <h2>Workers</h2>
                <p>The people that deliver this to you</p>
                {thisWorkers?.map((worker: Worker, index)=>{
                    return(<div key={index}>
                        <h3>{worker.name}</h3>
                        <p>{worker.auto_accept == true ? "When you book a time slot with this team member, your meeting is instantly confirmed. You will receive a calendar invite and a meeting link right away, so we can jump straight into discussing your project at your chosen time.": "After you submit your request, this team member will review their schedule and confirm your meeting shortly. You will receive a confirmation email with all the meeting details as soon as the request is accepted."}</p>
                    </div>)
                })}
            </div>
        </div>
